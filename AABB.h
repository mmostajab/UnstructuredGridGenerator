...
public:
    ...
    AABB_INLINE bool contains( const float point[3], glm::uint8_t splitAxis ) const;
  ...
  AABB_INLINE glm::uint32_t maxExtentAxis() const;
...
AABB_INLINE bool AABB::contains( const float point[3], glm::uint8_t splitAxis ) const
{
  const glm::uint8_t xAxis = (splitAxis + 2) % 3;
  const glm::uint8_t yAxis = (splitAxis + 4) % 3;
...
AABB_INLINE glm::uint32_t AABB::maxExtentAxis() const
{
  glm::vec3 dims( max[0] - min[0], max[1] - min[1], max[2] - min[2] );
